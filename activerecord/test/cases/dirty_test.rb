...
class DirtyTest < ActiveRecord::TestCase
  ...
  def test_save_should_store_serialized_attributes_even_with_partial_writes
    with_partial_writes(Topic) do
      with_autosave_serialized_attributes(Topic, true) do
  ...
  def test_save_should_not_store_serialized_attributes_when_set_unsafe
    with_partial_writes(Topic) do
      with_autosave_serialized_attributes(Topic, false) do
        topic = Topic.create!(:content => {:a => "a"})
        topic.content[:b] = "b"
        topic.save!
        topic.reload
  ...
  def test_serialized_columns_should_be_maybe_changed_if_safe
    with_autosave_serialized_attributes(Topic) do
      Topic.create!(:author_name => 'Bill', :content => {:a => "a"})
      topic = Topic.first
  ...
  def test_serialized_columns_should_be_maybe_changed_if_unsafe
    with_autosave_serialized_attributes(Topic, false) do
      assert_equal false, Topic.autosave_serialized_attributes?
      Topic.create!(:author_name => 'Bill', :content => {:a => "a"})
      topic = Topic.first
  ...
  private
    ...
    def with_autosave_serialized_attributes(klass, on = true)
      old = klass.autosave_serialized_attributes?
      klass.autosave_serialized_attributes = on
      yield
    ensure
      klass.autosave_serialized_attributes = old
    end
